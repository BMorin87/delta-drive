...
export const createUpgradeStore = (gameStore) =>
  create((set, get) => ({
    // The state.
    upgrades: {
    ...
    canAffordUpgrade: (upgradeId) => {
      ...
      return gameStore.getState().volition >= cost;
    },
    ...
    purchaseUpgrade: (upgradeId) => {
      ...
      // Spend volition
      gameStore.getState().spendVolition(cost);
      ...
      if (upgradeId.toLowerCase().includes("capacity")) {
        const baseProp =
          "base" + upgradeId[0].toUpperCase() + upgradeId.slice(1);
        const baseValue = gameStore.getState()[baseProp] ?? 100;
